class ProjectView {
  ...
  createProjectCard(
    ...
    projectID,
    deletable = true,
    iconSrc = "./img/note.svg"
  ) {
    const card = this.createElement("div", [
      "card",
      "project-bordered",
      "blue-background-transit",
    ]);
  ...
  toggleProject(id) {
    const projectCards = Array.from(
      this.projectsList.getElementsByClassName("card")
    );
  ...
  bindDeleteProject(handler) {
    // Get delete buttons
    let projectDeletes = this.projectsList.getElementsByClassName(
      "card-content--action-icons delete"
    );
    Array.from(projectDeletes).forEach((del) => {
      del.addEventListener("click", (event) => {
        const projectID = Number(
          event.target.parentNode.parentNode.dataset.projectId
        );
