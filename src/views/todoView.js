class TodoView {
  ...
  displayTodoForm(todo, projects) {
    ...
    const closeIcon = this.createElement("img", ["todo-form__icon", "close-form"]);
    ...
    if (todo) {titleInput.value = todo.title}  // Check this
    ...
    high_option.value = "3",
    high_option.innerText = "High"
    if (todo && todo.priority == high_option.value) { high_option.selected = true}
  
    const medium_option = this.createElement("option", ["text-warning"])
    medium_option.value = "2",
    medium_option.innerText = "Medium"
    if (todo && todo.priority == medium_option.value) { medium_option.selected = true}

    const low_option = this.createElement("option", ["text-success"])
    low_option.value = "1",
    low_option.innerText = "Low"
    if (todo && todo.priority == low_option.value) { low_option.selected = true}
    ...
    // Description
    const descriptionGroup = this.createElement("div", ["form-group", "col-md-11"]);
    ...
    if (todo) {descriptionInput.innerText = todo.description}  // Check this
  ...
  bindAddTodo(handler) {
    this.formContainer.firstElementChild.addEventListener("submit", event => {
      ...
      if (!event.target.classList.contains("for-edit")) {
        ...
        if ([title.value, project.value, dueDate.value, priority.value, description.value].every(val => val !== "")) {
          handler(
            title.value,
            description.value,
            dueDate.value,
